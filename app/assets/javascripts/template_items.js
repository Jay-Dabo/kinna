app.controller('template_item_form_ctrl', function ($scope) {
	...
	$scope.select_accounting_group = function() {
        for (x=0; x < gon.accounting_groups.length; x++) {
            if (gon.accounting_groups[x].number == $scope.accounting_group) {
                ...
                $('#template_item_account').val($scope.accounts[0].number);
                $('#template_item_description').val($scope.accounts[0].name);
            }
        }
	};
    $scope.select_account = function() {
        for (x=0; x < $scope.accounts.length; x++) {
            if ($scope.accounts[x].number == $scope.acc) {
                $('#template_item_account').val($scope.accounts[x].number);
                $('#template_item_description').val($scope.accounts[x].name);
            }
        }
    };
});